...
SELECT CUSTOMER_ID,
       ...
       CASE
           ...
           WHEN REPORTING_DATE IS NULL THEN FALSE -- reporting_date is null
           ...
           ELSE TRUE
           END                                           AS IS_VALID,
       (CASE WHEN CUSTOMER_ID <= 0 THEN 'CUSTOMER_ID' ELSE '' END) || '~' ||
       ...
       '~' ||
       (CASE WHEN BUCKET <= 0 THEN 'BUCKET' ELSE '' END) AS INVALID_COLUMNS
FROM client_stage;
